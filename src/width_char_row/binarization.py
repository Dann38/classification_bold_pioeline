...
def _adjust_gamma(image: np.ndarray, gamma: float = 1.2):  # noqa
...
def _postprocess(image: np.ndarray) -> np.ndarray:
...
def _get_block_index(image_shape, yx, block_size):  # noqa
...
def _adaptive_median_threshold(img_in: np.ndarray) -> np.ndarray:
...
# of the regions.
def _block_image_process(image, block_size):  # noqa
    out_image = np.zeros_like(image)
    for row in range(0, image.shape[0], block_size):
        for col in range(0, image.shape[1], block_size):
            idx = (row, col)
            block_idx = _get_block_index(image.shape, idx, block_size)
            out_image[tuple(block_idx)] = _adaptive_median_threshold(image[tuple(block_idx)])
    return out_image


# This function invokes the whole pipeline of Step 2.
def _get_mask(img: np.ndarray) -> np.ndarray:
    image_in = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    image_in = _preprocess(image_in)
    image_out = _block_image_process(image_in, BLOCK_SIZE)
    image_out = _postprocess(image_out)
    return image_out


# This is the function used for composing
def _sigmoid(x, orig, rad):  # noqa
...
def _combine_block(img_in, mask):  # noqa
    ...
    f = (v / (r + 1e-5))
    f = _sigmoid(f, bound_value + 0.05, 0.2)
...
def _combine_block_image_process(image, mask, block_size):  # noqa
    out_image = np.zeros_like(image)
    for row in range(0, image.shape[0], block_size):
        for col in range(0, image.shape[1], block_size):
            idx = (row, col)
            block_idx = _get_block_index(image.shape, idx, block_size)
            out_image[tuple(block_idx)] = _combine_block(
                image[tuple(block_idx)], mask[tuple(block_idx)])
    return out_image


# The main function of this section. Executes the whole pipeline.
def binarize(img: np.ndarray, block_size: int = 20) -> np.ndarray:
    img = _adjust_gamma(img)
    mask = _get_mask(img)
    image_in = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    image_out = _combine_block_image_process(image_in, mask, block_size)  #40 - у главной страницыб было 20)
